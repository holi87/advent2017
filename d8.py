...
t dec -949 if uy <= -2989""".split("\n")
max_lista = []
lista_zmiennych = set()
slownik = {}
for row in instructions:
    row = row.split()
    lista_zmiennych.add(row[0])
for element in lista_zmiennych:
    slownik[element] = 0


def dec_or_inc(row_inside):
    row_inside[2] = int(row_inside[2])
    if row_inside[1] == "dec":
        slownik[row_inside[0]] -= row_inside[2]
    else:
        slownik[row_inside[0]] += row_inside[2]
    max_lista.append(slownik[row_inside[0]])
...
print("part 1: %d, part 2: %d" % (max(slownik.values()), max(max_lista)))
